                        ...
                        uint64_t *tb_bytes_per_period)
{
    ...
    if (period >= RTE_METER_TB_PERIOD_MIN) {
        ...
        *tb_bytes_per_period = ceil(RTE_METER_TB_PERIOD_MIN / period);
                       ...
                       struct rte_meter_srtcm_profile *p)
{
    ...
    /* Initialize srTCM run-time structure */
    m->time = cycles_counter_update__();
    m->tc = p->cbs;
    m->te = p->ebs;
                       ...
                       struct rte_meter_trtcm_profile *p)
{
    ...
    /* Initialize trTCM run-time structure */
    m->time_tc = m->time_tp = cycles_counter_update__();
    m->tc = p->cbs;
    m->tp = p->pbs;
    ...
    struct rte_meter_trtcm_rfc4115_profile *p)
{
    ...
    /* Initialize trTCM run-time structure */
    m->time_tc = m->time_te = cycles_counter_update__();
    m->tc = p->cbs;
    m->te = p->ebs;
