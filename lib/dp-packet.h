...
dp_packet_set_rss_hash(struct dp_packet *p, uint32_t hash)
{
    ...
    *dp_packet_ol_flags_ptr(p) |= DP_PACKET_OL_RSS_HASH_MASK;
}

static inline bool
dp_packet_rss_valid(const struct dp_packet *p)
{
    return *dp_packet_ol_flags_ptr(p) & DP_PACKET_OL_RSS_HASH_MASK;
}

static inline void
dp_packet_reset_offload(struct dp_packet *p)
{
    *dp_packet_ol_flags_ptr(p) &= DP_PACKET_OL_NONE;
}
